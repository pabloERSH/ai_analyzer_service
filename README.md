...
**Микросервис интеллектуального анализа питания и тренировок для Telegram Mini Apps.**

Собирает данные о питании и тренировках, получает агрегированные недельные отчёты от Diary Service и формирует персонализированные рекомендации с помощью GigaChat LLM (Сбер). Асинхронная обработка через Celery + Redis. Построен на FastAPI, Redis и PostgreSQL.

---

## ✨ Возможности

- 🔐 **JWT-аутентификация** — единый токен для всех микросервисов (Auth → Diary → Analyzer)
- 📊 **Умная агрегация** — получение структурированных недельных отчётов из Diary Service
- 🧠 **AI-анализ** — GigaChat интерпретирует данные питания и тренировок по кастомному промпту
- ⚡ **Асинхронность** — Celery + Redis для неблокирующей обработки задач
- 💾 **Кэширование** — Redis для повторных запросов, PostgreSQL для хранения истории
- 🐳 **Docker** — полная контейнеризация с nginx-шлюзом
- 📝 **Логирование** — ротация логов в файлы + вывод в stdout
...
```
Telegram Mini App → Nginx Gateway → AI Analyzer Service (FastAPI)
                                        ├── Celery Worker
                                        ├── Redis (брокер + кэш)
                                        ├── PostgreSQL (история)
                                        ├── Diary Service (источник данных)
                                        └── GigaChat API (LLM)
```
